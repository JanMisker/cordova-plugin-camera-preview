...
public class CameraSource {
  ...
  /**
   * Jan Misker july 2017
   * Use the supplied camera and starts sending preview frames to the underlying detector.  The preview
   * frames are not displayed.
   *
   * @throws IOException if the camera's preview texture or display could not be initialized
   */
  @RequiresPermission(Manifest.permission.CAMERA)
    public CameraSource start(Camera camera) throws IOException {
      mCamera = camera;
      Camera.Parameters parameters = camera.getParameters();
      Camera.Size camSize = parameters.getPreviewSize();
      mPreviewSize = new Size(camSize.width, camSize.height);
